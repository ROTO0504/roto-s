...
authRoute.post("/register/options", async (c) => {
  if (!(await allowRegister(c.env, c.req.header("cookie"), c.req.header("x-invite-token") ?? "")))
...
passkeysRoute.get("/", async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT credential_id, label, created_at, last_used_at FROM passkeys ORDER BY created_at DESC",
  ).all()
