    ...
    <div>
      <header className={css({ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', mb: '6' })}>
        ...
        <div className={css({ display: 'flex', gap: '2' })}>
          {!editing && (
            <button type="button" onClick={() => setEditing(true)} className={button({ size: 'sm', variant: 'outline' })}>
              Edit
            </button>
          )}
      ...
      <div className={card() + ' ' + css({ mb: '6' })}>
        {editing ? (
          <div className={css({ display: 'flex', flexDirection: 'column', gap: '3' })}>
            ...
            <div className={css({ display: 'flex', gap: '2' })}>
              <button type="button" onClick={save} className={button({ size: 'sm' })}>Save</button>
              <button type="button" onClick={() => setEditing(false)} className={button({ size: 'sm', variant: 'ghost' })}>
                Cancel
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <a href={stats.link.url} target="_blank" rel="noopener noreferrer" className={css({ wordBreak: 'break-all' })}>
              {stats.link.url}
            </a>
      ...
      {rows.length === 0 ? (
        ...
        <ul className={css({ display: 'flex', flexDirection: 'column', gap: '1' })}>
          {rows.slice(0, 8).map((r) => (
            ...
            >
              <span className={css({ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxW: '60%' })}>
                {r.key || '—'}
              </span>
              <span>{r.clicks}</span>
            </li>
