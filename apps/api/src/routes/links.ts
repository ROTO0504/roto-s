...
linksRoute.post("/", async (c) => {
  ...
  if (body.slug) {
    ...
    const exists = await c.env.DB.prepare("SELECT slug FROM links WHERE slug = ?").bind(body.slug).first()
...
linksRoute.get("/", async (c) => {
  const { results } = await c.env.DB.prepare("SELECT * FROM links ORDER BY created_at DESC LIMIT 200").all<Link>()
...
linksRoute.patch("/:slug", async (c) => {
  ...
  const result = await c.env.DB.prepare(`UPDATE links SET ${sets.join(", ")} WHERE slug = ?`)
    .bind(...params)
    .run()
